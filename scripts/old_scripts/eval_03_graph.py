...
def main(provider, force_thoughts=False, model=None):
    ...
    print("=" * 60)
    print(" Graph Query benchmarking test")
    print("=" * 60)
    ...
    for item in toxicology_queries:
        ...
        tables_needed = extract_tables_from_query(nl_query, all_tables, llm)
        graph_context = graph_navigator(tables_needed)

        enriched_query = f"Historical Context from Graph RAG:\n{graph_context}\n\nUse this context to solve the following question:\nCurrent Question: {nl_query}"
    ...
    if accuracies:
        ...
        save_results(results=final_export, output_file="graph_toxicology.json", base_folder="logs")
        print("All results saved successfully in 'logs/graph_toxicology.json'")
