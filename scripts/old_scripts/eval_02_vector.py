#!/usr/bin/env python3
"""
Test script for natural language to SparkSQL conversion using RAG (Leave-One-Out).

Usage:
    python test_rag_query.py
    python test_rag_query.py --force-thoughts
    python test_rag_query.py --provider openai --model o3-mini
    python test_rag_query.py --provider openai --model o3-mini --force-thoughts
...
Requirements:
    ...
    - Vector database populated in db/vector_store/
"""
...
def main(provider, force_thoughts=False, model=None):
    ...
    print("=" * 60)
    print(" RAG Query benchmarking test (Leave-One-Out)")
    print("=" * 60)
    ...
    for item in toxicology_queries:
        ...
        rag_context = get_similar_queries_context(nl_query, query_id)

        enriched_query = f"{rag_context}Use the examples above to solve the following question:\nCurrent Question: {nl_query}"
    ...
    if accuracies:
        ...
        final_export = {
            "summary_metrics": {
                "experiment": "rag_toxicology_leave_one_out",
        ...
        save_results(results=final_export, output_file="rag_toxicology.json", base_folder="logs")
        print("All results saved successfully in 'logs/rag_toxicology.json'")
